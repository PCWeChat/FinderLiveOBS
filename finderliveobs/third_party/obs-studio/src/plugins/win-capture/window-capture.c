...
enum window_class_type { none = 0, others, ppt, ppt_player, word, excel };
...
struct window_capture {
	...
	float check_ppt_player_window_timer;

	HWND window;
	RECT last_rect;
    char *office_file;
	enum window_class_type type; // window_class_type
...
static void update_settings(struct window_capture *wc, obs_data_t *s)
{
	...
	wc->office_file = obs_data_get_string(s, "office_file");

    if (wc->class != NULL) {
		if (stricmp(wc->class, "PPTFrameClass") == 0) {
			wc->type = ppt;
		} else if (stricmp(wc->class, "screenClass") == 0) {
			wc->type = ppt_player;
		} else if (stricmp(wc->class, "OpusApp") == 0) {
			wc->type = word;
		} else if (stricmp(wc->class, "XLMAIN") == 0) {
			wc->type = excel;
		} else {
			wc->type = others;
		}
...
static void wc_tick(void *data, float seconds)
{
	...
	wc->check_ppt_player_window_timer += seconds;
	if (wc->type == ppt && wc->window != NULL &&
	    wc->check_ppt_player_window_timer > WC_CHECK_TIMER) {
		wc->check_ppt_player_window_timer = 0;
		HWND player_window = NULL;

		// ppt window, try to find match ppt player window
		//
		// ! notice one powerpoint.exe will have multi ppt window.
		// so we should use the title to match not the class
		player_window =
			(wc->method == METHOD_WGC)
				? find_window_top_level(
					  INCLUDE_MINIMIZED,
					  WINDOW_PRIORITY_PPT_PLAYER_RULE,
					  "screenClass", wc->office_file,
					  wc->executable)
		...
		if (player_window != NULL) {
			wc->window = player_window;
			bfree(wc->class);
			bfree(wc->title);
			wc->title = decode_str(wc->office_file);
			wc->class = decode_str("screenClass");
			wc->type = ppt_player;
	...
	if (!wc->window || !IsWindow(wc->window)) {
        ...
        if (wc->type == ppt_player && wc->window != NULL) {
			// if ppt_player window invalid, try to find match ppt window
			bfree(wc->class);
			bfree(wc->title);
			wc->class = decode_str("PPTFrameClass");
			wc->title = decode_str(wc->office_file);
			wc->priority = WINDOW_PRIORITY_PPT_RULE;
			wc->type = ppt;
		}
