			 ...
			 bool generic_class)
{
	...
	} else if (priority == WINDOW_PRIORITY_PPT_RULE) {
		// ppt window rule.
		// same class, then compare title.
		// check the file name in title is same
		if (class_matches) {
			const char *cur_file = get_ppt_file_from_title(
				cur_title.array, cur_title.len);
			if (cur_file && title) {
				val = (stricmp(cur_file, title) == 0)
					      ? 0
					      : 0x7FFFFFFF;
				bfree(cur_file);
			}
		}
	} else if (priority == WINDOW_PRIORITY_PPT_PLAYER_RULE) {
		// ppt player window rule.
		// same as above
		if (class_matches) {
			const char *cur_file = get_player_file_from_title(
				cur_title.array, cur_title.len);
			if (cur_file && title) {
				val = (stricmp(cur_file, title) == 0)
					      ? 0
