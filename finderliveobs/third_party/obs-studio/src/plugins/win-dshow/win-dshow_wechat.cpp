...
// wechat add
#define VIDEO_BEAUTIFY    "video_beautify"
...
// wechat add
#define TEXT_BEAUTIFY       obs_module_text("Beautify")
...
struct DShowInput {
	...
	// wechat add
	typedef void (*video_beautify_callback)(void *source, void *data,
						size_t size, uint32_t format);
	bool beautify = false;
	video_beautify_callback VideoBeautifyCallbackFunc = nullptr;
	...
	inline ~DShowInput()
	{
		// wechat add
		beautify = false;
		VideoBeautifyCallbackFunc = nullptr;
			     ...
			     long long endTime, long rotation)
{
	...
	// wechat add
	if (beautify && VideoBeautifyCallbackFunc) {
		VideoBeautifyCallbackFunc(source, &frame, size,
					  (uint32_t)videoConfig.format);
	} else {
		obs_source_output_video2(source, &frame);
	}
...
{
	// wechat add
	beautify = obs_data_get_bool(settings, VIDEO_BEAUTIFY);
	if (beautify) {
		VideoBeautifyCallbackFunc =
			reinterpret_cast<DShowInput::video_beautify_callback>(
				obs_data_get_int(settings,
						 "VideoBeautifyCallback"));
		blog(LOG_INFO, "start init beautify");
	}
...
static obs_properties_t *GetDShowProperties(void *obj)
{
  ...
  // wechat add
	obs_properties_add_bool(ppts, VIDEO_BEAUTIFY, TEXT_BEAUTIFY);
