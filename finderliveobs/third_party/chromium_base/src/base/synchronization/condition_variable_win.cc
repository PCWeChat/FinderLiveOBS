...
ConditionVariable::ConditionVariable(Lock* user_lock) {
  static const bool use_vista_native_cv = BindVistaCondVarFunctions();
  if (use_vista_native_cv) {
    impl_ = base::MakeUnique<WinVistaCondVar>(user_lock);
  } else {
    impl_ = base::MakeUnique<WinXPCondVar>(user_lock);
  }
}
//} wechat_support_xp
