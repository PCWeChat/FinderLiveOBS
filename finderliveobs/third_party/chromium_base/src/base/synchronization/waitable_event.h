...
//wechat {
#define MANUAL_NOT_SIGNALED_WAITABLE_EVENT()\
base::WaitableEvent event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)

#define SCOPED_WAITABLE_EVENT(event)                      \
struct WaitableEventSignal {                              \
  void operator() (base::WaitableEvent* waitable_event) { \
    if (waitable_event) {                                 \
      waitable_event->Signal();                           \
    }                                                     \
  }                                                       \
};                                                        \
std::unique_ptr<base::WaitableEvent, WaitableEventSignal> scoped(event)
//} wechat
