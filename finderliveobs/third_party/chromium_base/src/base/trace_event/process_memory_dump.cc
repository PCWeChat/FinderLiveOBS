...
size_t ProcessMemoryDump::CountResidentBytes(void* start_address,
  ...
  while (offset < mapped_size) {
    ...
    using QueryWorkingSetExPtr = decltype(::QueryWorkingSetEx)*;
    // QueryWorkingSetEx in kernel32.dll on windows7 and windows server 2008 r2
    // and in psapi.dll on windows server 2008, windows vista windows server 2003 and windows xp
    static const QueryWorkingSetExPtr query_working_set_ex_temp = reinterpret_cast<QueryWorkingSetExPtr>(\
      ::GetProcAddress(::GetModuleHandleW(L"kernel32.dll"), "QueryWorkingSetEx"));
    static const QueryWorkingSetExPtr query_working_set_ex = (query_working_set_ex_temp ? query_working_set_ex_temp :
      reinterpret_cast<QueryWorkingSetExPtr>(::GetProcAddress(::GetModuleHandleW(L"psapi.dll"), "QueryWorkingSetEx")));
    if (query_working_set_ex) {
      failure = !query_working_set_ex(GetCurrentProcess(), vec.get(), vec_size);
    }
    else {
      failure = true;
    }
