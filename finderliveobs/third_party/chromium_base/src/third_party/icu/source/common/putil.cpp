...
//wechat_support_xp_tomo {
using GetUserDefaultLocaleNamePtr = decltype(::GetUserDefaultLocaleName)*;
GetUserDefaultLocaleNamePtr GetUserDefaultLocaleNameFunc() {
  static const GetUserDefaultLocaleNamePtr get_user_default_locale_name_func =
    reinterpret_cast<GetUserDefaultLocaleNamePtr>(::GetProcAddress(::GetModuleHandleW(L"Kernel32.dll"), 
      "GetUserDefaultLocaleName"));
  return get_user_default_locale_name_func;
}

U_CAPI const char* U_EXPORT2
wechat_support_xp_uprv_getDefaultLocaleID() {
#if U_POSIX_LOCALE
  /*
...
{
  if (nullptr == GetUserDefaultLocaleNameFunc()) {
    return wechat_support_xp_uprv_getDefaultLocaleID();
  }
#if U_POSIX_LOCALE
/*
  Note that:  (a '!' means the ID is improper somehow)
...
#define POSIX_LOCALE_CAPACITY 64
    ...
    //wechat_support_xp_tomo {
    int length = GetUserDefaultLocaleNameFunc()(windowsLocale, UPRV_LENGTHOF(windowsLocale));
    //} wechat_support_xp_tomo
#else
